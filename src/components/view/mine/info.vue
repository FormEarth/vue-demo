<template>
  <mu-container>
    <mu-paper :z-depth="0">
      <div class="user-card" :style="header">
        <div class="user-top" style="height:60px;text-align:left;">
        ...
        <div class="user-name">
          {{user.name}}
          <van-button plain type="danger" size="mini" style="font-weight: normal;">已关注</van-button>
        </div>
        <div class="user-other">
          <div class="user-mini-card">
            ...
            <div>
              <span>个人简介:</span>这个人还没有填写简介哦
            </div>
          </div>
        </div>
      </div>
    </mu-paper>
    <mu-paper class="user-data">
    ...
    <mu-tabs :value.sync="active" full-width inverse color="indigoA200" indicator-color="redA700" text-color="rgba(0, 0, 0, .54)" center>
      <mu-tab :ripple="false">
        <mu-icon value="library_books"></mu-icon>
      </mu-tab>
      <mu-tab :ripple="false">
        <mu-icon value="burst_mode"></mu-icon>
      </mu-tab>
      <mu-tab :ripple="false">
    ...
    <div class="demo-text" v-if="active === 2">
      <div v-if="favorites==''" style="text-align:center;margin-top:4px;">还没有喜欢哦</div>
      <div></div>
    </div>
  </mu-container>
</template>
